...
@implementation cdaXSellEndTableCell
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        self.headerLabel.font = [UIFont fontWithName:@"Arial-BoldMT" size:((UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone) ? 16 : 18)];
        ...
        self.descriptionLabel.font =[UIFont fontWithName:@"Arial" size:((UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone) ? 14 : 16)];
        ...
        self.downloadHereLabel.font =[UIFont fontWithName:@"Arial-BoldMT" size:((UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone) ? 14 : 16)];
...
-(id) setAppValues:(cdaXSellApp *) app forTable:(UITableView *) table {
    ...
    self.headerLabel.text = [self.headerLabel.text stringByReplacingOccurrencesOfString:@"\n" withString:@" "];
...
-(CGFloat) textXOffset {
    return self.iconImageView.frame.size.width+((UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone) ? T_IMAGE_PADDING_RIGHT_IPHONE :T_IMAGE_PADDING_RIGHT_IPAD);
}
...
-(CGFloat) sizeContentToFit {
    ...
    totalSize += ((UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone) ? T_HEADER_PADDING_BOTTOM_IPHONE : T_HEADER_PADDING_BOTTOM_IPAD);
    totalSize += [self sizeLabelToFit:self.descriptionLabel];
    totalSize += [self sizeLabelToFit:self.downloadHereLabel];
    totalSize += T_DOWNLOAD_HERE_PADDING_BOTTOM;
    ...
    totalSize += ((UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone) ? T_CELL_PADDING_BOTTOM_IPHONE : T_CELL_PADDING_BOTTOM_IPAD);
...
- (void)layoutSubviews {
    ...
    self.descriptionLabel.frame = CGRectMake(boundsX+headerLabel.frame.origin.x, (headerLabel.frame.origin.y+headerLabel.frame.size.height+((UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone) ? T_HEADER_PADDING_BOTTOM_IPHONE - 1 : T_HEADER_PADDING_BOTTOM_IPAD -4)), descriptionLabel.frame.size.width, descriptionLabel.frame.size.height); //-4 to compensate uilabel padding
    self.downloadHereLabel.frame =CGRectMake(boundsX+descriptionLabel.frame.origin.x, (descriptionLabel.frame.origin.y + descriptionLabel.frame.size.height+T_DOWNLOAD_HERE_PADDING_BOTTOM), downloadHereLabel.frame.size.width, downloadHereLabel.frame.size.height);
}
