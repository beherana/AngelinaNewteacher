...
#define MA_IMAGE_PADDING_RIGHT_IPAD   20
#define MA_HEADER_PADDING_BOTTOM_IPAD 12
#define MA_DESCRIPTION_PADDING_BOTTOM_IPAD 6

#define MA_IMAGE_PADDING_RIGHT_IPHONE   20
#define MA_HEADER_PADDING_BOTTOM_IPHONE 6
#define MA_IMAGE_PADDING_LEFT_IPHONE  20
#define MA_DESCRIPTION_PADDING_BOTTOM_IPHONE 6
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.headerLabel.font = [UIFont fontWithName:@"Arial-BoldMT" size:((UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone) ? 13 : 19)];
...
- (void)showLeavingAppAlert
{
    ...
    if (![NetworkUtils connectedToNetwork]) {
        [alert show:[[[self superview] superview] superview] alertType:CAVCNoNetwork];
    }
    else {
        [alert show:[[[self superview] superview] superview] alertType:CAVCLeaveAppAppStore];
...
-(void) appTapped {
    [[cdaAnalytics sharedInstance] trackEvent:[NSString stringWithFormat:@"X Sell Object tap", self.headerLabel.text] inCategory:@"End Page: More Apps" withLabel:@"title" andValue:-1];
...
-(CGFloat) textXOffset {
    return self.iconImageView.frame.size.width+((UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone) ? MA_IMAGE_PADDING_RIGHT_IPHONE :MA_IMAGE_PADDING_RIGHT_IPAD);
}

-(CGFloat) textYOffset {
    return -4; //lift the header label upp a bit
...
-(CGFloat) sizeContentToFit {
    ...
    totalSize += ((UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone) ? MA_HEADER_PADDING_BOTTOM_IPHONE : MA_HEADER_PADDING_BOTTOM_IPAD);
    totalSize += [self sizeLabelToFit:self.descriptionLabel];
    totalSize += ((UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone) ? MA_DESCRIPTION_PADDING_BOTTOM_IPHONE : MA_DESCRIPTION_PADDING_BOTTOM_IPAD);
...
- (void)layoutSubviews {
    ...
    self.descriptionLabel.frame = CGRectMake(boundsX+headerLabel.frame.origin.x, (headerLabel.frame.origin.y+headerLabel.frame.size.height+((UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone) ? MA_HEADER_PADDING_BOTTOM_IPHONE - 1 : MA_HEADER_PADDING_BOTTOM_IPAD -4)), descriptionLabel.frame.size.width, descriptionLabel.frame.size.height); //-4 to compensate uilabel padding
    self.downloadHereLabel.frame =CGRectMake(boundsX+descriptionLabel.frame.origin.x, (descriptionLabel.frame.origin.y + descriptionLabel.frame.size.height+((UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone) ? MA_DESCRIPTION_PADDING_BOTTOM_IPHONE : MA_DESCRIPTION_PADDING_BOTTOM_IPAD)), downloadHereLabel.frame.size.width, downloadHereLabel.frame.size.height);
}
