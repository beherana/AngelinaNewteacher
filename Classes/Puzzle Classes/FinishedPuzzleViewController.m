...
static int __lastRandom = -1;

@interface FinishedPuzzleViewController()
@end

@implementation FinishedPuzzleViewController 
...
- (void)viewDidLoad
{
    ...
    NSDictionary *plist = [NSDictionary dictionaryWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"puzzle_reward" ofType:@"plist"]];
    
    int limit = [[plist objectForKey:@"maxRandom"] intValue] - 1;
    int divisor = RAND_MAX/(limit+1);
    int random;
    do { 
        random = arc4random() / divisor;
    } while (random > limit && random != __lastRandom);
    __lastRandom = random;
    self.images = [NSMutableDictionary dictionary];
    self.audio = [NSMutableArray array];
    
    NSArray *animations = [plist objectForKey:@"animations"];
        
    for (NSDictionary *animation in animations) {
        NSNumber *rand = [animation objectForKey:@"random"];
        if (rand != nil && [rand intValue] != random) {
            continue;
        }
    ...
    for (NSDictionary *audio in [plist objectForKey:@"audio"]) {
        NSNumber *rand = [audio objectForKey:@"random"];
        if (rand != nil && [rand intValue] != random) {
            continue;
        }
        
        [self.audio addObject:[audio objectForKey:@"filename"]];
...
- (void)startAnimation
{
    ...
    for (NSDictionary *audio in [plist objectForKey:@"audio"]) {
        for (NSString *filename in self.audio) {
            if ([filename isEqualToString:[audio objectForKey:@"filename"]]) {
                NSURL *url = [[NSBundle mainBundle] URLForResource:filename withExtension:@"m4a"];
                [self performSelector:@selector(playAudio:) withObject:url afterDelay:[[audio objectForKey:@"delay"] floatValue]];
            }
        }
    }
...
{
    AVQueueItem *queueItem = [[AVQueueManager sharedAVQueueManager] enqueueAudioFileUrl:url withPrio:2000 exclusive:YES userData:url];
