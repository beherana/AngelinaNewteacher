...
+ (NSUInteger) ask: (NSString *) question withTitle: (NSString *) ttl withCancel: (NSString *) cancelButtonTitle withButtons: (NSArray *) buttons
{
	...
	UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:ttl message:question delegate:madelegate cancelButtonTitle:cancelButtonTitle otherButtonTitles:nil];
...
+ (void) say: (id)formatstring,...
{
	...
	va_end(arglist);
	[ModalAlert ask:statement withTitle:nil withCancel:@"Okay" withButtons:nil];
	[statement release];
}
...
+ (BOOL) ask: (id)formatstring,...
{
	...
	id statement = [[NSString alloc] initWithFormat:formatstring arguments:arglist];
	va_end(arglist);
	BOOL answer = ([ModalAlert ask:statement withTitle:nil withCancel:nil withButtons:[NSArray arrayWithObjects:@"No", @"Yes",  nil]] == 1);
	[statement release];
	return answer;
}
...
+ (BOOL) confirm: (id)formatstring,...
{
	...
	id statement = [[NSString alloc] initWithFormat:formatstring arguments:arglist];
	va_end(arglist);
	BOOL answer = [ModalAlert ask:statement withTitle:nil withCancel:@"Cancel" withButtons:[NSArray arrayWithObject:@"OK"]];
	[statement release];
	return	answer;
}
